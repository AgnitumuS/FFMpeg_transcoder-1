...
//-----------------------------
void FFMPEG::SetupVideo(char * filename, int Width, int Height, int FPS, int GOB, int BitPerSecond) {
...
void FFMPEG::WriteFrame(char * RGBFrame) {
	...
	for (int y=0;y<m_c->height;y++) {  //Height Loop
		...
		for (int x=0;x<m_c->width;x++) { //Width Loop
			...
			//Save RGB frame to FFMPEG's source frame
			m_src_picture.data[0][y * m_src_picture.linesize[0] + x*3+0] = RGBFrame[(y*m_c->width+x)*3+2];  //Red Channel
			m_src_picture.data[0][y * m_src_picture.linesize[0] + x*3+1] = RGBFrame[(y*m_c->width+x)*3+1];  //Green Channel
			m_src_picture.data[0][y * m_src_picture.linesize[0] + x*3+2] = RGBFrame[(y*m_c->width+x)*3+0];  //Blue Channel
		}
	}
